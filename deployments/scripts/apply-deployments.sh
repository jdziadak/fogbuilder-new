...
helm install -n prometheus kube-prometheus-stack prometheus-community/kube-prometheus-stack --version 67.9.0 \
  --wait \
  --set "defaultRules.create=false" \
  --set "nodeExporter.enabled=false" \
  --set "prometheu s.prometheusSpec.ruleSelectorNilUsesHelmValues=false" \
...
helm install rabbitmq-old bitnami/rabbitmq --version 15.2.2 \
  --namespace default \
  ...
  --set metrics.serviceMonitor.default.enabled=true \
  --set metrics.serviceMonitor.detailed.enabled=true \
  --set metrics.serviceMonitor.perObject.enabled=true
...
#kubectl patch svc my-couchdb-svc-couchdb -n couchdb -p '{"spec": {"ports": [{"nodePort": 30984, "port": 5984}]} }' #todo expsoe service
...
kubectl patch svc couchdb-np -n couchdb -p '{"spec": {"ports": [{"nodePort": 30984, "port": 5984}]} }'

kubectl patch svc node-red-np -n node-red -p '{"spec": {"ports": [{"nodePort": 30001, "port": 1880}]} }'
